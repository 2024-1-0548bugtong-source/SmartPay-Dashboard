...
const float COIN_RESET_THRESHOLD = 6.0;
const int COIN_STABLE_SAMPLES_REQUIRED = 2;
...
int stableCoinCandidate = 0;
int stableCoinSamples = 0;
bool coinEventLatched = false;
...
void loop() {
  ...
  if (productType > 0 && previousProductType == 0) {
    ...
    stableCoinCandidate = 0;
    stableCoinSamples = 0;
    coinEventLatched = false;
  ...
  if (!checkoutActive || coinW <= COIN_RESET_THRESHOLD) {
    stableCoinCandidate = 0;
    stableCoinSamples = 0;
    coinEventLatched = false;
  } else if (!coinEventLatched) {
    if (coinValue == stableCoinCandidate) {
      stableCoinSamples++;
    } else {
      stableCoinCandidate = coinValue;
      stableCoinSamples = 1;
    }
  }

  bool coinReadingStable = checkoutActive && !coinEventLatched && coinValue > 0 && stableCoinSamples >= COIN_STABLE_SAMPLES_REQUIRED;
  bool invalidWeightStable = checkoutActive && !coinEventLatched && coinValue < 0 && stableCoinSamples >= COIN_STABLE_SAMPLES_REQUIRED;

  if (coinReadingStable) {
    int expectedCoin = requiredAmount;
    if (coinValue != expectedCoin) {
      char coinDetectedMsg[96];
      snprintf(coinDetectedMsg, sizeof(coinDetectedMsg), "Coin Detected: %.1fg -> PHP%d", coinW, coinValue);
      sendMessage(coinDetectedMsg);
      sendMessage("PAYMENT INVALID");
      sendMessage("payment status: Insufficient");
    ...
    coinEventLatched = true;
  }

  if (invalidWeightStable) {
    ...
    sendMessage("PAYMENT INVALID");
    sendMessage("payment status: No coin detected");
    invalidCoinState = true;
    coinEventLatched = true;
  }
...
/* ================= COIN DETECTION ================= */
int detectCoin(float weight) {
  if (weight <= COIN_RESET_THRESHOLD) return 0; // no coin / scale reset
  if (weight >= 33 && weight <= 39) return 5;   // ₱5 stable band
  if (weight >= 42 && weight <= 50) return 10;  // ₱10 stable band
  return -1; // coin present but outside valid thresholds
}
